...
start(Num_Clients, Cl_Nodes, TG_Nodes, Client_DBs, Done) ->
	...
	{split_db_finished, ok} ->
	    if
		Done + 1 == length(TG_Nodes) ->
		    timer:sleep(1000), %% <== NASTY HACK
...
%% @end
%%--------------------------------------------------------------------
setup_dbs([], _Container_Pids, _Client_DBs, _Start_Pid) ->
    ok;
setup_dbs([TG_Node|Tail_TG_Nds], Container_Pids, Client_DBs, Start_Pid) ->
...
%% @end
%%--------------------------------------------------------------------
setup_dbs([], _Container_Pid, _Start_Pid) ->
    ok;
setup_dbs([H|T], Container_Pid, Start_Pid) ->
...
split_ets(List, Table_Name, Num_Tables) ->
    ...
    [insert_elements(X, Tables, Limit) || X <- List],
    Tables.
...
%% @end
%%--------------------------------------------------------------------
get_client_DBs([], Client_DBs) ->
    Client_DBs;    
get_client_DBs([Node|Tail_Nodes], Client_DBs) ->
