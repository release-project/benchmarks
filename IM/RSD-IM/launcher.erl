...
-export([start/5, start/6, start_bencherl/1, start_bencherl/5, stop/5, stop/6,
	 launch_router_processes/7, launch_server_supervisors/5]).

-import(router,[router_supervisor/1, router_process/1, compression_function/2, hash_code/2]).
...
start(Servers_Per_Router_Node, Servers_Per_Router_Process, Servers_Total, Clients_Total, List_Domains, Num_of_Hosts) ->
	...
	false ->
	    ...
	    case whereis(routers_listener) of
	    	undefined ->
	    	    io:format("~n=============================================~n" ++
				"Initiating the Distributed Instant Messenger.~n"++
				"=============================================~n"),
...
start_host(Num_of_Host, Architecture_Info, Domain, Routers_Listener_Pid) ->
    ...
    io:format("~n=============================================~n" ++
		...
		"Server nodes: ~p~n" ++
		"Client nodes: ~p~n" ++
		"Router processes: ~p~n",[Num_of_Host,
					  Router_Nodes,
					  Server_Nodes,
					  Client_Nodes, 
					  Router_Processes]),
    ...
    timer:sleep(3000), %% <==== INSERTED DELAY HERE
    spawn(hd(Router_Nodes), fun() -> s_group:new_s_group(routers_group, Router_Nodes) end),
...
start_bencherl(Nodes) ->
    ...
    {Router_Nodes, Server_Nodes, Client_Nodes, _Traffic_Nodes} = extractor(Nodes),
...
launch_router_supervisors(Routers_Servers_Lists, Architecture_Info, Routers_Listener_Pid) ->
	...
	true ->
	    ...
	    io:format("launch_router_supervisors.~n" ++ 
			  "Router_Nd: ~p~n" ++
			  "Servers in ~p:~p~n" ++
			  "Router Processes in ~p:~p~n", [Router_Nd,
							  Router_Nd,
							  Servers,
							  Router_Nd,
							  Router_Prcs]),
...
start_router_supervisor(Router_Node, Server_Nodes, Servers_Router_Pr, Router_Processes, Num_Total_Servers, Routers_Listener_Pid) ->
    S_Group_Name = server_s_group_name(Router_Node),
    timer:sleep(3000),  %% <==== INSERTED DELAY HERE
    spawn(Router_Node, fun() -> s_group:new_s_group(S_Group_Name, [Router_Node]) end),

    R_Sup_Conf = {S_Group_Name, undefined, Router_Processes, [], [], [], [], [], []},
    R_Sup_Pid = spawn_link(Router_Node, fun() ->
						router_supervisor(R_Sup_Conf)
					end),

    Routers_Listener_Pid ! {R_Sup_Pid, add_router_sup},
    Router_Info = {Server_Nodes,
		   Servers_Router_Pr,
		   Router_Processes,
		   Num_Total_Servers,
		   Routers_Listener_Pid,
    ...
    R_Sup_Pid ! {Router_Info, launch_router_processes},
    ok.
...
%% @end
%%--------------------------------------------------------------------
launch_router_processes(S_Group_Name, R_Sup_Pid, Server_Nodes, Servers_Router_Pr, Router_Processes, Num_Total_Servers, Routers_Listener_Pid) ->
	...
	[Router_Process|New_Router_Processes] ->
		...
		true ->
		    ...
		    start_router_process(S_Group_Name,
					 Router_Process,
					 Servers, Servers_Router_Pr,
					 Num_Total_Servers,
					 R_Sup_Pid,
					 Routers_Listener_Pid),
		    launch_router_processes(S_Group_Name,
					    R_Sup_Pid,
					    New_Server_Nodes,
					    Servers_Router_Pr,
...
%% @end
%%--------------------------------------------------------------------
start_router_process(S_Group_Name, Router_Name, Server_Nodes, Servers_Router_Pr, Num_Total_Servers, R_Sup_Pid, Routers_Listener_Pid) ->
    %% s_group:register_name(S_Group_Name, Router_Name,
    %% 			  R_Pid = spawn_link(fun() -> 
    %% 						     router_process({initial_state, R_sup_Pid, [], [], []})
    %% 					     end)),
    R_Pid = spawn_link(fun() ->
			       router_process({initial_state, S_Group_Name, R_Sup_Pid, [], [], []})
		       end),
    Routers_Listener_Pid ! {S_Group_Name, Router_Name, R_Pid, add_router},
    Servers_Information = {S_Group_Name,
			   Router_Name,
			   Server_Nodes,
			   Servers_Router_Pr,
			   Num_Total_Servers,
			   Routers_Listener_Pid},
    R_Pid ! {Servers_Information, launch_server},
    yes.
...
%% @end
%%--------------------------------------------------------------------
launch_server_supervisors(S_Group_Name, Server_Nodes, Servers_Router_Pr, Num_Total_Servers, Routers_Listener_Pid) ->
    case length(Server_Nodes) > Servers_Router_Pr of
	true ->
	    {Servers, New_Server_Nodes} = lists:split(Servers_Router_Pr, Server_Nodes),
	    io:format("launch_server_supervisors.~nServers:~p~n", [Server_Nodes]),
	    start_server_supervisors(S_Group_Name, Servers, Num_Total_Servers, Routers_Listener_Pid),
	    launch_server_supervisors(S_Group_Name,
				      New_Server_Nodes,
				      Servers_Router_Pr,
				      Num_Total_Servers,
				      Routers_Listener_Pid);
	false ->
	    io:format("launch_server_supervisors.~nServers:~p~n", [Server_Nodes]),
	    start_server_supervisors(S_Group_Name,
				     Server_Nodes,
				     Num_Total_Servers,
				     Routers_Listener_Pid)
    end.
...
%% @end
%%--------------------------------------------------------------------
start_server_supervisors(S_Group_Name, Server_Nodes, Num_Total_Servers, Routers_Listener_Pid) ->
	...
	false ->
	    ...
	    timer:sleep(3000), %% <==== INSERTED DELAY HERE.
	    s_group:add_nodes(S_Group_Name, [Server]),
	    timer:sleep(3000),  %% <==== INSERTED DELAY HERE.
	    spawn(Server, fun() -> self_checker(S_Group_Name, Server, s_group:registered_names({s_group, S_Group_Name}),0, 0) end),
	    Server_Sup_Pid = spawn_link(Server, fun() -> 
							server:start_server_supervisor(S_Group_Name,
										       Server,
										       string_to_atom(Server_Name), 
										       Num_Total_Servers)
						end),
	    Routers_Listener_Pid ! {S_Group_Name, Server_Name, Server_Sup_Pid, add_server},
	    start_server_supervisors(S_Group_Name, New_Server_Nodes, Num_Total_Servers, Routers_Listener_Pid)
    end.


%%--------------------------------------------------------------------
%% @doc
%%     self_checker/5 tests whether all the relay names are registered
%%     and accessible. It runs these tests for five times. If the tests
%%     fail after five attempts, it notifies the error to the stdout.
%%
%% @spec self_checker(S_Group_Name, Server, Processes, Acc, Trials -> ok.
%% @end
%%--------------------------------------------------------------------
self_checker(S_Group_Name, Server, [], Acc, Trials) ->
    case Acc of
	50 -> %% <===== CHANGE IF THE NUMBER OF RELAYS IS INCREASED OR REDUCED (CURRENTLY 50)
	    io:format("~p is ok~n",[Server]),
	    ok;
	_Other ->
	    N_Trial = Trials + 1,
	    io:format("~p failed~n",[Server]),
	    if 
		N_Trial == 6 ->
		    io:format("Server ~p failed to get the relays.~n", [Server]),
		    ok;
		N_Trial == 1 ->
		    Router_Node = hd(s_group:own_nodes()),
		    spawn(Router_Node, fun() -> s_group:add_nodes(S_Group_Name, [Server]) end),
		    timer:sleep(1000),
		    self_checker(S_Group_Name, Server, s_group:registered_names({s_group, S_Group_Name}),0, N_Trial);
		true ->
		    Router_Node = hd(s_group:own_nodes()),
		    spawn(Router_Node, fun() -> s_group:remove_nodes(S_Group_Name, [Server]) end),
		    timer:sleep(1000 * N_Trial),
		    spawn(Router_Node, fun() -> s_group:add_nodes(S_Group_Name, [Server]) end),
		    timer:sleep(1000 * N_Trial),
		    self_checker(S_Group_Name, Server, s_group:registered_names({s_group, S_Group_Name}),0, N_Trial)
	    end
    end;
self_checker(S_Group_Name, Server, [H|T], Acc, Trials) ->
    {_,Proc} = H,
    case string:substr(atom_to_list(Proc),1,6) of
	"server" ->
	    self_checker(S_Group_Name, Server, T, Acc, Trials);
	_Other ->
	    self_checker(S_Group_Name, Server, T, Acc + 1, Trials)
    end.

%%====================================================================
%% AUTO STOP APPLICATION LOGIC
%%====================================================================
%%--------------------------------------------------------------------
%% @doc
%%     auto_stop/1 stops and closes the IM when the benchmarks are
%%     finished.
%%
%% @spec auto_stop(Nodes) -> fun()
%% @end
%%--------------------------------------------------------------------
auto_stop(Nodes) ->
    {_Router_Nodes, _Server_Nodes, Client_Nodes, _Traffic_Nodes} = extractor(Nodes),
    Auto_Stop_Pid = self(),
    launch_stop_notifiers(Client_Nodes, Auto_Stop_Pid),
    auto_stop(Nodes, length(Client_Nodes), 0).

%%--------------------------------------------------------------------
%% @doc
%%     auto_stop/3 stops and closes the IM when the benchmarks are
%%     finished.
%%
%% @spec auto_stop(Nodes, Total_Loggers, Finished_Loggers) -> pid().
%% @end
%%--------------------------------------------------------------------
auto_stop(Nodes, Total_Loggers, Finished_Loggers) ->
    receive
	logger_stopped ->
	    case Finished_Loggers + 1 of
		Total_Loggers ->
		    spawn(fun() -> stop_nodes_gently(Nodes) end);
		New_Finished_Loggers ->
		    auto_stop(Nodes, Total_Loggers, New_Finished_Loggers)
	    end;
	_Other ->
	    io:format("Something failed in auto_stop/3"),
	    auto_stop(Nodes, Total_Loggers, Finished_Loggers)
    end.

%%--------------------------------------------------------------------
%% @doc
%%     launch_stop_notifiers/2 sets up the processes that notify the
%%     end of the benchmarks.
%% 
%% @spec launch_stop_notifiers(Client_Nodes, Auto_Stop_Pid) -> ok
%% @end
%%--------------------------------------------------------------------
launch_stop_notifiers(Client_Nodes, Auto_Stop_Pid) ->
	...
	[] ->
	    ok;
	[Node|New_Client_Nodes] ->
	    spawn(Node, fun() -> create_notifier(Auto_Stop_Pid) end),
	    launch_stop_notifiers(New_Client_Nodes, Auto_Stop_Pid)
    end.

%%--------------------------------------------------------------------
%% @doc
%%     create_notifier/1 is the first stage of the notifier process.
%%     This process notifies the auto_stop process that a logger is
%%     stopped.
%%
%% @spec create_notifier(Auto_Stop_Pid) -> fun().
%% @end
%%--------------------------------------------------------------------
create_notifier(Auto_Stop_Pid) ->
    register(stop_notifier, self()),
    notifier(Auto_Stop_Pid).

%%--------------------------------------------------------------------
%% @doc
%%     notifier/1 is the second stage of the notifier process. This
%%     process notifies the auto_stop process that a logger is stopped.
%%
%% @spec notifier(Auto_Stop_Pid) -> logger_stopped
%% @end
%%--------------------------------------------------------------------
notifier(Auto_Stop_Pid) ->
    receive
	logger_stopped ->
	    Auto_Stop_Pid ! logger_stopped;
	_Other ->
	    io:format("something failed with the notifier~n"),
	    notifier(Auto_Stop_Pid)
    end.
...
%%--------------------------------------------------------------------
%% @doc
%%     stop/5 stops the application by shutting down all nodes.
%%
%% @spec stop(S_RN, S_RP, S_T, Cl_T, List_Domains) -> ok
%% @end
%%--------------------------------------------------------------------
...
%%--------------------------------------------------------------------
%% @doc
%%     stop/6 stops the application by shutting down all nodes.
%%
%% @spec stop(Servers_Per_Router_Node, Servers_Per_Router_Process,
...
%%--------------------------------------------------------------------
%% @doc
%%     stop_host/3 stops an IM that has been deployed locally in one
%%     host.
%%
%%     Traffic_Nodes is hard-coded to 4. This has an easy fix adding
%%     Traffic_Total as parameter. This is not done because it is used
%%     in local tests and debugging.
%%
%% @spec stop_host(Num_of_Host, Architecture_Info, Domain) -> ok
%% @end
%%--------------------------------------------------------------------
stop_host(Num_of_Host, Architecture_Info, Domain) ->
    ...
    Server_Nodes = node_name_generator(Num_of_Host, server, Domain, Servers_Total, 0),
    Client_Nodes = node_name_generator(Num_of_Host, client, Domain, Clients_Total, 0),
    Traffic_Nodes = node_name_generator(Num_of_Host, traffic, Domain, 4, 0),

    stop_nodes(Traffic_Nodes),
...
%%--------------------------------------------------------------------
%% @doc
%%     stop_nodes/1 stop nodes in the list passed as parameter.
%%
%% @spec stop_nodes(Nodes_List) -> ok
%% @end
%%--------------------------------------------------------------------
...
%%--------------------------------------------------------------------
%% @doc
%%     stop_nodes_gently/1 stop nodes in the list passed as parameter.
%%     The nodes are stopped in order, so the running processes are
%%     stopped before node is finished.
%%
%% @spec stop_nodes(Nodes_List) -> ok
%% @end
%%--------------------------------------------------------------------
stop_nodes_gently(Nodes_List) ->
    {Router_Nodes, Server_Nodes, Client_Nodes, Traffic_Nodes} = extractor(Nodes_List),
    stop_nodes(Client_Nodes),
    stop_nodes(Traffic_Nodes),
    finish_nodes({router, Router_Nodes}),
    finish_nodes({server, Server_Nodes}),
    init:stop().

%%--------------------------------------------------------------------
%% @doc
%%     finish_nodes/1 spawn the corresponding finish_node/0 functions
%%     to finish router and server nodes gently.
%%
%% @spec finish_nodes({Node_Type, [H|T]}) -> ok.
%% @end
%%--------------------------------------------------------------------
finish_nodes({_Node_Type, []}) ->
    ok;
finish_nodes({Node_Type, [H|T]}) ->
    case Node_Type of
	router ->
	    spawn(H, fun() -> router:finish_node() end),
	    finish_nodes({router, T});
	server ->
	    spawn(H, fun() -> server:finish_node() end),
	    finish_nodes({server, T})
    end.
...
routers_listener(Servers_Total, List_Servers, List_Routers, List_Router_Sups, Client_Nodes) ->
    receive
	{S_Group_Name, Server_Name, Server_Sup_Pid, add_server} ->
	    New_List_Servers = [{S_Group_Name, Server_Name, Server_Sup_Pid} | List_Servers],
		...
		true ->
		    ...
		    Final_List_Routers = lists:reverse(List_Routers),
		    feed_router_sup(List_Router_Sups, Final_List_Routers, lists:reverse(Routers_DBs_Pids)),
		    feed_routers(New_List_Servers, Final_List_Routers, Final_List_Routers);
		false ->
		    routers_listener(Servers_Total, New_List_Servers, List_Routers, List_Router_Sups, Client_Nodes)
	    end;
	{S_Group_Name, Router_Name, Router_Pid, add_router} ->
	    New_List_Routers = [{S_Group_Name, atom_to_list(Router_Name), Router_Pid} | List_Routers],
	    routers_listener(Servers_Total, List_Servers, New_List_Routers, List_Router_Sups, Client_Nodes);
	{Router_Sup_Pid, add_router_sup} ->
	    New_List_Router_Sups = [Router_Sup_Pid | List_Router_Sups],
...
feed_routers(List_Servers, List_Routers, List_Routers_2) ->
	...
	%%init:stop();
	[{_, _, Router_Pid}|New_List_Routers] ->
...
%%
%% @spec extractor(Nodes_List) -> {list(), list(), list(), list()}.
%% @end
%%--------------------------------------------------------------------
...
%% @end
%%--------------------------------------------------------------------
extractor(Nodes_List, R_Nodes, S_Nodes, Cl_Nodes, Tr_Nodes) ->
    case Nodes_List of
	[] ->
	    {lists:reverse(R_Nodes),
	     lists:reverse(S_Nodes),
	     lists:reverse(Cl_Nodes),
	     lists:reverse(Tr_Nodes)};
	[H|T] ->
	    case string:sub_string(atom_to_list(H), 1, 3) of
		"rou" ->
		    New_R_Nodes = [H|R_Nodes],
		    extractor(T, New_R_Nodes, S_Nodes, Cl_Nodes, Tr_Nodes);
		"ser" ->
		    New_S_Nodes = [H|S_Nodes],
		    extractor(T, R_Nodes, New_S_Nodes, Cl_Nodes, Tr_Nodes);
		"cli" ->
		    New_Cl_Nodes = [H|Cl_Nodes],
		    extractor(T, R_Nodes, S_Nodes, New_Cl_Nodes, Tr_Nodes);
		"tra" ->
		    New_Tr_Nodes = [H|Tr_Nodes],
		    extractor(T, R_Nodes, S_Nodes, Cl_Nodes, New_Tr_Nodes);
		_Other ->
		    extractor(T, R_Nodes, S_Nodes, Cl_Nodes, Tr_Nodes)
	    end
    end.
...
%%--------------------------------------------------------------------
%% @doc
%%     server_s_group_name/1 is an auxiliary function to generate the
%%     server_s_group names.
%%
%% @spec server_s_group_name(Router_Node) -> string()
%% @end
%%--------------------------------------------------------------------
server_s_group_name(Router_Node) ->
    R_Name = string:sub_word(atom_to_list(Router_Node), 1, $@),
    R_Num = string:sub_word(R_Name, 2, $_),
    string_to_atom("server_group_" ++ R_Num).
