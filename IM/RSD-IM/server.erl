...
%%% @doc
%%%	Server module for the Reliable Scalable Distributed Erlang instant
%%%     messenger (RSD-IM) application developed as a real benchmark for
...
-export([start_server_supervisor/4, stop_server_supervisor/1, restart_server_supervisor/1, finish_node/0, compression_function/2]).
...
%% @end
%%---------------------------------------------------------------------
start_server_supervisor(S_Group_Name, Node, Server_Name, Num_Total_Servers) ->
    ...
    register_dbs(S_Group_Name, Monitored_DBs),
    ...
    server_supervisor_loop(S_Group_Name, Server_Name, Client_DBs, Chat_DBs, Monitored_Processes, Num_Total_Servers).
...
%%
%% @spec start_server_supervisor({first_stage, S_Group_Name, Server_Name}) ->
%%          Pid | {error, Reason}
%% @end
%%---------------------------------------------------------------------
restart_server_supervisor({first_stage, S_Group_Name, Server_Name, Num_Total_Servers}) ->
    process_flag(trap_exit, true),
    ...
    restart_server_supervisor({second_stage, S_Group_Name, undefined, Server_Name, {Mon_Process_Table, monitored_processes_2}, Num_Total_Servers});

restart_server_supervisor({second_stage, S_Group_Name, _Div_Pid, Server_Name, Monitored_Processes, Num_Total_Servers}) ->
...
%%
%% @spec server_supervisor_loop(S_Group_Name, Server_Name, Client_DBs,
%%                              Chat_DBs, Monitored_Processes,
%%                              Num_Total_Servers) -> no_return()
%% @end
%%--------------------------------------------------------------------
server_supervisor_loop(S_Group_Name, Server_Name, Client_DBs, Chat_DBs, Monitored_Processes, Num_Total_Servers) ->
	...
	%% ================ DEBUG ================
	{request_DBs, From} ->
	    From ! {{client_DBs, Client_DBs},{chat_DBs, Chat_DBs}},
	    server_supervisor_loop(S_Group_Name, Server_Name, Client_DBs, Chat_DBs, Monitored_Processes, Num_Total_Servers);
	%% =======================================
	%%client login request.
	{Client_Name, Client_Pid, login} ->
	    Server_Supervisor_Pid = self(),
	    {Client_Monitor_Pid, _Cl_Mon_Ref} = spawn_monitor(fun() -> client_monitor(S_Group_Name,
										      Client_Name,
										      Client_Pid,
										      [],
										      Client_DBs,
	...
	{Sender, Receiver, start_chat_session} ->
	    {Chat_Session_Pid, _Ch_Ses_Ref} = spawn_monitor(fun() -> chat_session(S_Group_Name,
										  Chat_DBs,
										  [{Sender, undefined},
										   {Receiver, undefined}],
										  Num_Total_Servers)
	...
	%% confirmation server supervisor is down without any problems.
	    ...
	    stop_dbs(S_Group_Name, Client_DBs, Chat_DBs);
	...
	{'DOWN', Ref, process, Pid, Reason} ->
		...
		"Client_DB" ->
		    Tokens = string:tokens(atom_to_list(Name), "_"),
	    	    {New_Client_DBs, New_Chat_DBs} = replicate_db(S_Group_Name, Pid, Tokens, Client_DBs, Chat_DBs, Monitored_Processes),
		    db_recovery(S_Group_Name, Tokens),
		    server_supervisor_loop(S_Group_Name, Server_Name, New_Client_DBs, New_Chat_DBs, Monitored_Processes, Num_Total_Servers);
		"Chat_DB" ->
		    Tokens = string:tokens(atom_to_list(Name), "_"),
	    	    {New_Client_DBs, New_Chat_DBs} = replicate_db(S_Group_Name, Pid, Tokens, Client_DBs, Chat_DBs, Monitored_Processes),
	    	    db_recovery(S_Group_Name, Tokens),
		    server_supervisor_loop(S_Group_Name, Server_Name, New_Client_DBs, New_Chat_DBs, Monitored_Processes, Num_Total_Servers);
		"Chat_Session" ->
		    chat_session_recovery({initial_state, S_Group_Name, Pid, Chat_DBs, Monitored_Processes}),
		    server_supervisor_loop(S_Group_Name, Server_Name, Client_DBs, Chat_DBs, Monitored_Processes, Num_Total_Servers);
		"Client_Monitor" ->
		    client_monitor_recovery(S_Group_Name, Pid, Client_DBs, self(), Monitored_Processes),
		    server_supervisor_loop(S_Group_Name, Server_Name, Client_DBs, Chat_DBs, Monitored_Processes, Num_Total_Servers)
	    end;
	{recovered, ok} ->
...
%%
%% @spec chat_session(S_Group_Name, Chat_DBs, Clients_Info,
%%                    Num_Total_Servers) -> no_return().
%% @end
%%--------------------------------------------------------------------
chat_session(S_Group_Name, Chat_DBs, Clients_Info, Num_Total_Servers) ->
	...
	{_, Sender, Receiver, start_chat_session} ->
	    ...
	    send_to_db(S_Group_Name, Chat_DB_Name, {self(), peak, {Session_Name}}),
	    chat_session(S_Group_Name, Chat_DBs, Clients_Info, Num_Total_Servers);
	%% Session setup.
	[] ->
	    {New_Client_A_Pid, client_pid} = find_client_pid(S_Group_Name, Client_A, Num_Total_Servers),
	    {New_Client_B_Pid, client_pid} = find_client_pid(S_Group_Name, Client_B, Num_Total_Servers),
	    case New_Client_B_Pid of
	    	client_not_found when is_pid(New_Client_A_Pid) ->
	    	    New_Client_A_Pid ! {receiver_not_found, Client_B};
		_ when New_Client_A_Pid == client_not_found ->
		    send_to_db(S_Group_Name, Chat_DB_Name, {undefined, remove, {Session_Name}}),
		    send_to_replica(S_Group_Name, Chat_DB_Replica, {undefined, remove, {Session_Name}}),
		    chat_session(S_Group_Name, Chat_DBs, Clients_Info, Num_Total_Servers);
	    	_ ->
	    	    New_Clients_Info = [{Client_A, New_Client_A_Pid},{Client_B, New_Client_B_Pid}],
	    	    send_to_db(S_Group_Name, Chat_DB_Name, {self(), add, {Session_Name, self(), Client_A, New_Client_A_Pid, Client_B, New_Client_B_Pid}}),
	    	    send_to_replica(S_Group_Name, Chat_DB_Replica, {undefined, add, {Session_Name, self(), Client_A, New_Client_A_Pid, Client_B, New_Client_B_Pid}}),
	    	    chat_session(S_Group_Name, Chat_DBs, New_Clients_Info, Num_Total_Servers)
	    end;
	...
	{_, _, _, finish_chat_session} ->
	    send_to_db(S_Group_Name, Chat_DB_Name, {self(), remove, {Session_Name}}),
	    send_to_replica(S_Group_Name, Chat_DB_Replica, {undefined, remove, {Session_Name}}),
	    chat_session(S_Group_Name, Chat_DBs, Clients_Info, Num_Total_Servers);
	%% Traffic generator (Toxic client).
...
%% @end
%%--------------------------------------------------------------------
client_monitor(S_Group_Name, Client_Name, Client_Pid, Opened_Chat_Sessions, Client_DBs, Server_Supervisor_Pid) ->
    process_flag(trap_exit, true),
    {Client_DB_Name, Client_DB_Replica} = Client_DBs,
    receive
	%% Login logic.
	{start_client_monitor} ->
	    case send_to_db(S_Group_Name, Client_DB_Name, {self(), peak, {Client_Name}}) of
		{error, _} ->
		    send_to_replica(S_Group_Name, Client_DB_Replica, {self(), peak, {Client_Name}});
		{ok, _} ->
		    ok
	    end,
	    receive
		[] ->
		    ...
		    send_to_db(S_Group_Name, Client_DB_Name, {self(), add, {Client_Name, Client_Pid, self()}}),
		    send_to_replica(S_Group_Name, Client_DB_Replica, {undefined, add, {Client_Name, Client_Pid, self()}}),
		    client_monitor(S_Group_Name, Client_Name, Client_Pid, Opened_Chat_Sessions, Client_DBs, Server_Supervisor_Pid);		    
		Response ->
	...
	{add_chat_session, Chat_Session} ->
	    ...
	    client_monitor(S_Group_Name, Client_Name, Client_Pid, New_Chat_Sessions, Client_DBs, Server_Supervisor_Pid);
	...
	{'DOWN', _Ref, process, _Pid, _Reason} -> 
	    case send_to_db(S_Group_Name, Client_DB_Name, {self(), peak_by_client_monitor, {self()}}) of
		{error, _} ->
		    send_to_replica(S_Group_Name, Client_DB_Replica, {self(), peak_by_client_monitor, {self()}});
		{ok, _} ->
		    ok
	    end,
	    receive
		[Client_Name, Client_Pid, _Client_Monitor_Pid] ->
		    send_to_db(S_Group_Name, Client_DB_Name, {self(), remove, {Client_Name}}),
		    send_to_replica(S_Group_Name, Client_DB_Replica, {undefined, remove, {Client_Name}}),
...
%%      a server supervisor.
%%
%% @spec register_dbs(S_Group_Name, DBs) -> ok
%% @end
%%--------------------------------------------------------------------
register_dbs(_S_Group_Name, []) ->
	    ok;
register_dbs(S_Group_Name, [{Name, Pid}|T_DBs]) ->
    case Name of
	monitored_processes_2 -> 
	    register(Name, Pid),
	    register_dbs(S_Group_Name, T_DBs);
	_Other ->
	    s_group:register_name(S_Group_Name, Name, Pid),
	    db_registration_checker(S_Group_Name, Name, Pid, 0),
	    register_dbs(S_Group_Name, T_DBs)
    end.

%%--------------------------------------------------------------------
%% @doc
%%     Auxiliary function to the register_dbs/2 function. It checks
%%     whether the name registration of the database has was successfull
%%     or not. In the case it was not, it tries to register the name
%%     up to five times. After five failures it returns an error.
%%
%% @spec db_registration_checker(atom(), atom(), pid(), integer()) ->
%%              ok | {error, reason}
%% @end
%%--------------------------------------------------------------------
db_registration_checker(_S_Group_Name, Name, _Pid, 5) ->
    io:format("DB ~p failed to register.~n",[Name]),
    {error, registration_failed};
db_registration_checker(S_Group_Name, Name, Pid, Acc) ->
    case s_group:whereis_name(S_Group_Name, Name) of
	undefined ->
	    s_group:register_name(S_Group_Name, Name, Pid),
	    timer:sleep(250),
	    db_registration_checker(S_Group_Name, Name, Pid, Acc + 1);
	Pid ->
	    ok
    end.
...
%% @end
%%--------------------------------------------------------------------
replicate_db(S_Group_Name, Failed_Pid, Tokens, Client_DBs, Chat_DBs, Monitored_Processes) ->
    {Monitored_Proc_Table, Monitored_Proc_DB} = Monitored_Processes,
	...
	"Client" ->
	    ...
	    s_group:register_name(S_Group_Name, DB_Name, New_DB_Pid),
	    %% ====> (MAYBE) ADD SOME SORT OF NAME CHECKER/TIMER OR SOMETHING <====
...
db_recovery(S_Group_Name, Tokens) ->
    ...
    send_to_replica(S_Group_Name, Source, {self(), recover, {S_Group_Name, Destination, Source}}).
...
%% @end
%%---------------------------------------------------------------------
chat_session_recovery({initial_state, S_Group_Name, Session_Pid, Chat_DBs, Monitored_Processes}) ->
    {Chat_DB_Name, Chat_DB_Replica} = Chat_DBs,
    case send_to_db(S_Group_Name, Chat_DB_Name, {self(), peak_by_pid, {Session_Pid}}) of
	{error, _} ->
	    send_to_replica(S_Group_Name, Chat_DB_Replica, {self(), peak_by_pid, {Session_Pid}}),
	    chat_session_recovery({final_state, S_Group_Name, Session_Pid, Chat_DBs, Monitored_Processes});
	{ok, _} ->
	    chat_session_recovery({final_state, S_Group_Name, Session_Pid, Chat_DBs, Monitored_Processes})
    end;
chat_session_recovery({final_state, S_Group_Name, Session_Pid, Chat_DBs, Monitored_Processes}) ->
...
%% @end
%%---------------------------------------------------------------------
client_monitor_recovery(S_Group_Name, Client_Monitor_Pid, Client_DBs, Server_Supervisor_Pid, Monitored_Processes) ->
    {Monitored_Proc_Table, Monitored_Proc_DB} = Monitored_Processes,
    {Client_DB_Name, Client_DB_Replica} = Client_DBs,
    case send_to_db(S_Group_Name, Client_DB_Name, {self(), peak_by_client_monitor, {Client_Monitor_Pid}}) of
    	{error, _} ->
	    send_to_replica(S_Group_Name, Client_DB_Replica, {self(), peak_by_client_monitor, {Client_Monitor_Pid}});
	{ok, _} ->
	    ok
    end,
    receive
	    ...
	    {New_Client_Monitor_Pid, _Ref} = spawn_monitor(fun() -> client_monitor(S_Group_Name,
										   Client_Name,
										   Client_Pid,
										   Chat_Sessions,
										   Client_DBs,
	    ...
	    send_to_db(S_Group_Name, Client_DB_Name, {undefined, update_monitor_pid, {Client_Name, New_Client_Monitor_Pid}}),
	    send_to_replica(S_Group_Name, Client_DB_Replica, {undefined, update_monitor_pid, {Client_Name, New_Client_Monitor_Pid}}),
...
stop_dbs(S_Group_Name, Client_DBs, Chat_DBs) ->
    ...
    exit(s_group:whereis_name(S_Group_Name, Cl_DB1),normal),
    exit(s_group:whereis_name(S_Group_Name, Ch_DB1),normal),
    case s_group:whereis_name(S_Group_Name, Cl_DB2) of
	undefined ->
	    ok;
	Pid1 ->
	    exit(Pid1,normal)
    end,
    case s_group:whereis_name(S_Group_Name, Ch_DB2) of
	undefined ->
	    ok;
	Pid2 ->
	    exit(Pid2,normal)
...
%% @end
%%---------------------------------------------------------------------
find_client_pid(S_Group_Name, Client_Name, Num_Total_Servers) ->
    ...
    case send_to_db(S_Group_Name, Client_DB_Main, {self(), client_pid, {Client_Name}}) of
	{error, db_is_down} ->
	    send_to_replica(S_Group_Name, Client_DB_Replica, {self(), client_pid, {Client_Name}});
	{ok, _Message} ->
	    ok
    end,
    receive
...
%% @end
%%---------------------------------------------------------------------
send_to_db(S_Group_Name, DB_Name, Message) ->
    case s_group:whereis_name(S_Group_Name, DB_Name) of
	undefined ->
	    {error, db_is_down};
	Pid ->
	    Pid ! Message,
...
send_to_replica(S_Group_Name, DB_Replica, Message) ->
	...
	undefined ->
	    ...
	    broadcast_message(S_Group_Name, Broadcast_Message);
	Pid ->
	    Pid ! Message
    end.
...
broadcast_message(S_Group_Name, Message) ->
    Relay = string_to_atom("relay_" ++ integer_to_list(random:uniform(50))), %% <=== CHANGE TO THE ACTUAL NUMBER OF RELAYS (NOW 50).
