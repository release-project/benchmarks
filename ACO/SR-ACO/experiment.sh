...
Erlang_path="/home/ag275/Desktop/temp/sderlang/bin";
...
if $Separate_Master_and_Starter; then
	...
	VMname="starter_node@";
	if $Run_printer; then
		erl -name ${VMname}${Hostnames[1]} -noinput -s aco run printer chaos $Instances_FileName $vertex_degree $duplicating $Num_Ants $Iter_Global $Iter_Local $Config_file -s init stop -pa ebin -pa chaos/ebin>>$Output_file_name
	else
