...
-export([orbit/3,
		 orbit/7,
         get_gens/1, get_master/1, get_gateway/1, get_workers/1, get_spawn_img_comp/1,
...
orbit(Gs, Xs, NumGateways, P, TimeOut, SpawnImgComp, Sub_masters) ->
	...
	Group_Hash_Table= start_submasters(Sub_masters, {[], 0}, {Gs, Xs, NumGateways, P, TimeOut, SpawnImgComp,Credit}), %% start submaster nodes and returns Group Hash Table
	lists:foreach(fun({Pid,_,_,_}) -> Pid ! {start, Group_Hash_Table} end, Group_Hash_Table), %% send Group_Hash_Table to all sub-master processes
	lists:foreach(fun({_,Pids,_,_}) -> send_list(Pids,{start, Group_Hash_Table}) end, Group_Hash_Table), %% send Group_Hash_Table to all gateway processes
	...
	lists:foreach(fun({Pid,_, _, _}) -> Pid ! {dump} end, Group_Hash_Table), %% after collecting all the credits, terminates all sub-master processes
	lists:foreach(fun({_,Pids, _, _}) -> send_list(Pids,{dump}) end, Group_Hash_Table), %% after collecting all the credits, terminates all gateway processes
	collect_orbit(ElapsedTime, length(Group_Hash_Table)). %% collects the Orbit information from all sub-master nodes
...
%% groups hash table is a list of tuples, i.e. {Pid of submaster process, Pid of gateway process, Start, TableSize}
start_submasters([], { Group_Hash_Table, _GlobalSize}, {_Gs, _Xs, _NumGateways, _P, _Timeout, _Spawn, _Credit}) ->
  Group_Hash_Table;

start_submasters([{Node, GroupName, TabSize}], { Group_Hash_Table, GlobalSize}, {Gs, Xs, NumGateways, P, Timeout, Spawn, Credit}) ->
  Pid = spawn_link(Node, sub_master, init,[Gs, Xs, P, Timeout, Spawn, Credit, self(), GroupName]),
...
start_submasters([{Node, GroupName, TabSize} | Sub_masters], { Group_Hash_Table, GlobalSize}, {Gs, Xs, NumGateways, P, Timeout, Spawn, Credit}) ->
  Pid = spawn_link(Node, sub_master, init,[Gs, Xs, P, Timeout, Spawn, Credit, self(), GroupName]),  
  Gateways=spawn_gateways(Node, NumGateways,[]), %% spawns a number of gateway processes
  ...
  start_submasters(Sub_masters, {New_Group_Hash_Table,New_GlobalSize}, {Gs, New_Xs, NumGateways, P, Timeout, Spawn, New_Credit}).
