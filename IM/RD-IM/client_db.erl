...
client_db_loop(DB_Name) ->
	...
	{From, recover, Target_DB_Name, Source_DB_Name} ->
	    spawn(fun() -> recover_db(From, Target_DB_Name, Source_DB_Name) end),
	    client_db_loop(DB_Name);
...
%% @end
%%---------------------------------------------------------------------
recover_db(From, Destination, Source) ->
    ets:safe_fixtable(Source, true),
    global:whereis_name(Destination) ! {undefined, From, recovery_binary, term_to_binary(ets:tab2list(Source))},
