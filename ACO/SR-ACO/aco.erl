-module(aco).
-export([run/0, run/1, start_aco/9, start_aco1/10]).
...
start_aco (Fname, Vertex_degree, Duplicating, Num_Ants, Iter_Global, Iter_Local, NodeFile, Params0, Recovery) ->
	if 
		Recovery==false ->
			process_flag(trap_exit, true);
			%global:register_name(starter,self());
		true-> ok
	end,
    ...
    #params{chaos=Chaos} = Params0,
    if 
	Chaos==true ->
	    io:format ("running chaos monkey...~n"),
	    lists:foreach (fun(Node) -> spawn(Node, start_chaos, run, []) end, Nodes);
	true ->
	    ok
    end,

    timer:tc (aco, start_aco1, [Num_Jobs, Vertex_degree, Duplicating, Num_Ants, Iter_Global, Iter_Local, Nodes, Inputs, Params, _Recovery=false]).

start_aco1 (Num_Jobs, Vertex_degree, Duplicating, Num_Ants, Iter_Global, Iter_Local, Nodes, Inputs, Params, Recovery) ->
  ...
  % Things could go badly here if we're recovering from a failure of
  % the master node; however, the auto option is mostly for testing.

	[Head|_Tail]=Nodes,
    MasterPID=spawn_link(Head, ant_master, run, 
			 [Num_Jobs, Vertex_degree, Duplicating, Num_Ants, Iter_Global, Iter_Local, Inputs, Params, Nodes, Recovery, self()]),
    loop(MasterPID, Num_Jobs, Vertex_degree, Duplicating, Num_Ants, Iter_Global,Iter_Local, Nodes, Inputs, Params).

loop(MasterPID, Num_Jobs, Vertex_degree, Duplicating, Num_Ants, Iter_Global, Iter_Local, Nodes, Inputs, Params)->
	receive
		{MasterPID, {Best_Cost, Best_Schedule}} ->
		...
		{'EXIT', MasterPID, Reason} -> io:format ("Failure of the master process with PID ~p and reason ~p ~n", [MasterPID, Reason]),
				       start_aco1 (Num_Jobs, Vertex_degree, Duplicating, Num_Ants, Iter_Global, Iter_Local, 
						  Nodes, Inputs, Params, _Recovery=true);
	{message, Message} ->
	    io:format ("message (~p): ~p ~n", [time(), Message]),
	    loop(MasterPID, Num_Jobs, Vertex_degree, Duplicating, Num_Ants,  Iter_Global,Iter_Local, Nodes, Inputs, Params)
	end.
...
run2(Args,Params) ->
	...
	["chaos"|Rest] ->
	    run2(Rest, Params#params{chaos=true});
	["printer"|Rest] ->
	    run2(Rest, Params#params{printer=true});
	[Fname,Vertex_degree0, Duplicating0, Num_Ants0, Iter_Global0, Iter_Local0, NodeFile] ->
	    ...
	    {Time1, {Time2, Result}} =
		timer:tc (aco, start_aco, [Fname, Vertex_degree, Duplicating, Num_Ants, Iter_Global, Iter_Local, NodeFile, Params, _Recovery=false]),
