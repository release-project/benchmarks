...
Run_chaos=false;
Run_printer=true;
Separate_Master_and_Starter=true;
...
do
	...
	cd $Root_path;
	if $BwlfCluster ; then
		Max_node=12; # available node at Heriot-Watt's beowulf cluster  
	else
		Max_node=6; # available node at Uppsala's kalkyl cluster  
	fi

	for Number_of_Erlang_Nodes in 23
	do     
		cd $Root_path;
		...
		cp -r *.erl *.hrl nodes.txt compile experiment.sh chaos ${Base_directory};
		cd $Base_directory;

		if $BwlfCluster ; then
			chmod 755 experiment.sh
			./experiment.sh $BwlfCluster $experiment $Total_nodes $Number_of_VMs_per_Nodes $vertex_degree $duplicating $Num_Ants $Iter_Global $Iter_Local $Instances_FileName $Number_of_Erlang_Nodes $pin $Separate_Master_and_Starter $Run_chaos $Run_printer;
			cd ..
			rm -rf bwlf_Nodes_*
		else
			...
			sbatch $Experiment_name $BwlfCluster $experiment $Total_nodes $Number_of_VMs_per_Nodes $vertex_degree $duplicating $Num_Ants $Iter_Global $Iter_Local $Instances_FileName $Number_of_Erlang_Nodes $pin $Separate_Master_and_Starter $Run_chaos $Run_printer;
		fi
