-module(aco).
-export([run/0, run/1, start_aco/6]).
...
start_aco (Fname, Num_Ants, Iter_Global, Iter_Local, NodeFile, Params0) ->
	...
	timer:tc (ant_master, run, [Num_Jobs, Num_Ants, Iter_Global, Iter_Local, Inputs, Params, Nodes]),
...
usage() -> 
    ...
    io:format (" repl=<int>: set repeat count for messages between colonies and master (for benchmarking: default is 1)~n"),
...
run2(Args,Params) ->
	...
	[ [$n, $= |V] |Rest] -> 
	    ...
	    run2(Rest, Params);
	[ [$r, $e, $p, $l, $= |V] |Rest] -> 
	    Repl = util:int_of_string(V),
	    run2(Rest, Params#params{repl=Repl});
	["R"|Rest] ->
	...
	[Fname, Num_Ants0, Iter_Global0, Iter_Local0, NodeFile] ->
	    ...
	    {Time1, {Time2, Result}} =
		timer:tc (aco, start_aco, [Fname, Num_Ants, Iter_Global, Iter_Local, NodeFile, Params]),
