...
-import(router,[router_supervisor/6, router_process/1, compression_function/2, hash_code/2]). %% <==== CHANGE router_supervisor/5 TO router_supervisor/1 WHEN USING THE NEW VERSION OF ROUTER MODULE.

%% -include("config.hrl"). <==== TO BE USED WITH THE NEW VERSION OF ROUTER MODULE. (NOT FINISHED YET).
...
start_router_supervisor(Router_Node, Server_Nodes, Servers_Router_Pr, Router_Processes, Num_Total_Servers, Routers_Listener_Pid) ->
    %%===================================================================================
    %% THIS IS ALL TO BE USED WITH THE NEW VERSION OF ROUTER MODULE (NOT FINISHED YET)
    %%===================================================================================
    %% Config = #config{pid=undefined,
    %% 		     mon_rtrs= Router_Processes,
    %% 		     rtrs_list=undefined,
    %% 		     rtrs_info=undefined,
    %% 		     rtrs_dbs=undefined}, %% <==== ARGUMENT CHANGED TO RECORD
    %% io:format("============================================~n" ++
    %% 	      " Config in launcher = ~p~n" ++
    %% 	      "============================================~n", [Config]),
    %% R_Sup_Pid = spawn_link(Router_Node, fun() ->
    %% 						router_supervisor({normal_state, Config})
    %% 					end),
    %% Routers_Listener_Pid ! {R_Sup_Pid, add_router_sup},
    %% R_Sup_Pid ! {launch_router_processes, Server_Nodes,
    %% 		 Servers_Router_Pr,
    %% 		 Router_Processes,
    %% 		 Num_Total_Servers,
    %% 		 Routers_Listener_Pid},
    %%===================================================================================
    R_Sup_Pid = spawn_link(Router_Node, fun() ->
						router_supervisor(undefined, Router_Processes,[], [], [], []) %%) <==== ADDED one more list (Servers_List).
					end),
    Routers_Listener_Pid ! {R_Sup_Pid, add_router_sup},
...
routers_listener(Servers_Total, List_Servers, List_Routers, List_Router_Sups, Client_Nodes) ->
	...
	{Server_Name, Server_Sup_Pid, add_server} ->
	    ...
	    if
		length(New_List_Servers) == Servers_Total ->
...
feed_routers(List_Servers, List_Routers, List_Routers_2) ->
	...
	[{_, Router_Pid}|New_List_Routers] ->
	    %% Router_Pid ! {list_routers_servers, List_Routers, List_Servers}, %% <==== TO BE USED WITH NEW VERSION OF ROUTER MODULE (NOT IMPLEMENTED YET).
