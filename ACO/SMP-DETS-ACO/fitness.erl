...
-spec edd(intlist(),intlist(),[idxf()]) -> [idxf()].
edd([], [], Acc) ->
    lists:reverse(Acc);
...
-spec mdd(intlist(), intvec(), intvec(), integer(), [idxf()]) -> [idxf()].
...
-spec au(intlist(), intvec(), intvec(), intvec(), integer(), float(), [idxf()]) -> [idxf()].
...
-spec au(intlist(), intvec(), intvec(), intvec(), integer()) -> [idxf()].
...
-spec make_eta(intlist(), heuristic(), inputs(), integer()) -> [idxf()].
...
%    io:format ("Cost=~p~nTime=~p~nDuration=~p~nDeadline=~p~nWeight=~p~nH=~p~nT=~p~n~n", 
%	       [Cost, Time, Duration, Deadline, Weight, HResult, TResult]),
